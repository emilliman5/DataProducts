...
tables <- mclapply(titles, mc.cores = 4, function(x) readHTMLTable(paste0(base,x)))
...
write.csv(obs, "UFO_sightings.txt", col.names = T, row.names = F, quote=T)
...
write.csv(geocodes[,c(3,1,2)], "geocodes_DSTK.txt", quote = T, col.names = T, row.names=F)
...
write.csv(obs.summary, "UFO_aggregate_sightings.txt", quote=T, col.names = T, row.names = F)
